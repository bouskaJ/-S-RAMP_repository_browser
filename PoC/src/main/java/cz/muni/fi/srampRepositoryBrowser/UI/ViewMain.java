	...
	public ViewMain(Composite parent, int style) {
		...
		importing.addListener (SWT.Selection, new Listener () {
			...
			public void handleEvent (Event event) {
				...
				if(selectionIndex != (-1)){
				final ArtifactSummary artifact = content.get(selectionIndex);
				...
				Job importing = new Job("importing artifact"){
					 ...
					 */
						public boolean shouldRun() {
							return super.shouldSchedule() && getManager().isConnected();
						}
						...
						protected IStatus run(IProgressMonitor monitor) {
							...
							try {
								...
								Display.getDefault().asyncExec(new Runnable() {
									public void run() {
										MessageDialog
												.openError(ViewMain.this.getShell(),
														"Importing failed!",
				...
				Job deleting = new Job("deleting artifact"){
					 ...
					 */
						...
						protected IStatus run(IProgressMonitor monitor) {
							...
							try {
								//deleting
								browserManager.deleteArtifact(artifact.getUuid(),artifact.getType());
								Job refresh = new RefreshJob("refreshing data",ViewMain.this);
								refresh.schedule();
